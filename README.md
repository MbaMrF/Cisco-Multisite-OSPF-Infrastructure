...
Este repositorio contiene la topología oficial, los ficheros de configuración, las tablas de estado dinámico y la documentación técnica de la infraestructura de red corporativa interconectada que unifica la sede principal de **Sants (Barcelona)** con las delegaciones de **Atocha (Madrid)** y **Triana (Sevilla)**.
...
### 🖼️ Topología Lógica del Entorno Corporativo
![Topología de Red Corporativa Multisensorial](NET%20TOPOLOGY.png)
...
## ⚙️ 2. Ficheros de Configuración y Estado de Dispositivos Críticos
...
vlan 30
 name VLAN_SISTEMAS
vlan 99
 name TRANSITO_HACIA_ROUTER
exit
🖼️ Verificación de la Base de Datos de VLANs (Core 1 & Core 2)
...
exit
🗺️ 3. Convergencia del Enrutamiento Dinámico (OSPF)
La comunicación global entre sedes utiliza OSPF de área única (Área 0). A continuación se documenta la tabla de adyacencias y el enrutamiento inyectado en los Cores.

🖼️ Tabla de Vecinos OSPF Activos en el Nodo Central
🖼️ Tabla de Rutas IP (show ip route) en Switches de Distribución
🛠️ 4. Bitácora de Ingeniería y Resolución de Problemas (Troubleshooting)
...
💻 5. Infraestructura de Servicios Generales e Integridad de Red
Los servidores de producción se conectan directamente al switch ACCESO-01-SANTS bajo direccionamiento estático en la VLAN 10, garantizando conectividad total y resolución de nombres hacia los puestos de usuario final.

🖼️ Verificación de Parámetros IP en Terminal de Usuario (Sants)
🖼️ Pruebas de Conectividad Extremo a Extremo (ICMP Ping)
